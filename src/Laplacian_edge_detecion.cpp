#include "Laplacian_edge_detecion.h"
#include <iostream>
using namespace std;

static PIXEL Lap(PIXEL WB[3][3])
{
	short int M00 = WB[0][1] ;
	short int M01 = WB[1][0] +WB[1][2];
	short int A00 = (WB[1][1] *4);
	short int S00 = (WB[2][1]);
	short int out_pix;
	out_pix = M01 + M00;
	out_pix = out_pix + S00;
	out_pix = A00-out_pix;
...
static PIXEL Lap_kernel(PIXEL WB[3][3])
{
	PIXEL lap;
	short temp;
	temp = Lap(WB);
...
void Laplacian_edge_detection(hls::stream<trans_pkt>& src, hls::stream<trans_pkt>& dst, int rows, int cols)
{
...
#pragma HLS pipeline
			...
			_sobel = Lap_kernel(WindowBuffer);
