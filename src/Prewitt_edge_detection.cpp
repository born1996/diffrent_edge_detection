#include "Prewitt_edge_detection.h"
#include <iostream>
using namespace std;

static PIXEL Gradient_X(PIXEL WB[3][3])
{
	short int M00 = ((short int)WB[1][0]);
	short int M01 = ((short int)WB[1][2]);
...
{
	short int M00 = ((short int)WB[0][1]);
	short int M01 = ((short int)WB[2][1]);
	short int A00 = (WB[2][0] + WB[2][2]);
	short int S00 = (WB[0][0] + WB[0][2]);
	short int out_pix;
	out_pix = out_pix + A00+M01;
	out_pix = out_pix - S00-M00;
